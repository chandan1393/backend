...
public class UserService {
    ...
    /**
     * Creates a student account if one does not already exist for this email.
     * Returns the User object with tempPassword set if newly created (null if existing).
     *
     * NOTE: Does NOT send any email — the calling service (QueryService) is
     * responsible for sending welcome + confirmation emails with full context.
    ...
    public User createUserFromQuery(String name, String email, String phone) {
        ...
        User user = User.builder()
            ...
            .enabled(true)
            .firstLogin(true)
            .tempPassword(tempPassword) // caller reads this to send welcome email
            .build();
